...
0.4f Spread core is an independent bottom-up strategy. V1 remains the performer melody; V4 is a root/slash-bass anchor; inner voices come from the Stage 4 candidate pool. Core Build #435 and full host Build #436 are green. Host integration activates `B0 / MIDI 35` without remapping other controls; User confirmed Spread works from a marked Clean/Color/Rich score in Studio Pro (2026-09-26). The user subsequently confirmed all other host checks passed and accepted 0.4f. The three levels show little pitch difference; refine later in Stage 5. Extreme-low-register host exercise was waived because trumpet range ends first; core MIDI safety regression passed. See `docs/TEST-0.4f.md` and `docs/0.4f-HOST-CHECKLIST.md`.
