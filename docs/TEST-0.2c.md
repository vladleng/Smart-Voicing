...
1. Windows CI успешно собирает оба VST3 и проходят `SmartVoicingCoreTests` + `SmartVoicingHarmonizerTests`.
2. Пользователь подтверждает в Studio Pro, что одна melody note создаёт ожидаемый четырёхголосный Close voicing по текущему Chord Track, non-chord melody не исправляется, а Direct Router не получил регрессий.
